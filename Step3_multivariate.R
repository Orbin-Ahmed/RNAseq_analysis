...
# Remember that PCA is unsupervised, so knows nothing about group assignment (Dexamethasone vs Vehicle)
...
mydata.df <- mutate(log2.cpm.filtered.norm.df,
                    Dexamethasone.AVG = (SCR1.1 + SCR1.2 + SCR2.1 + SCR2.2 + SCR3.1 + SCR3.2 + SCR4.1 + SCR4.2 + SCR5.1 + SCR5.1)/10, 
                    Vehicle.AVG = (SiP1.1 + SiP1.2 + SiP2.1 + SiP2.2 + SiP3.1 + SiP3.2 + SiP4.1 + SiP4.2 + SiP5.1 + SiP5.2)/10,
                    #now make columns comparing each of the averages above that you're interested in
                    LogFC = (Vehicle.AVG - Dexamethasone.AVG)) %>% #note that this is the first time you've seen the 'pipe' operator
  mutate_if(is.numeric, round, 2)
